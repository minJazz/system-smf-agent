...
public class GrowthManagementServiceImple implements GrowthManagementService {
	...
	public void manageGrowth() {
		...
		if (true) {
			...
			System.out.println(
					"{ { \"T\" : \"" + year + "-" + String.format("%02d", month) + "-" + String.format("%02d", date)
							+ " " + String.format("%02d", hour) + ":" + String.format("%02d", minute) + ":"
							+ String.format("%02d", second) + "\", \"H\" : \"" + h + "\", \"C\" : \"" + c);
			...
			if (c >= 20 && c <= 30) {
				if (h > setting.getHumidity() + 3) {
					controlHumidified(false);
					controlFan(true);
					System.out.println("h fan on");
					controlHeater(false);
					System.out.println("h heaterFan off");
				} else if (h <= setting.getHumidity() + 3 && h >= setting.getHumidity() - 3) {
					controlHumidified(false);
					controlHeater(false);
					System.out.println("h heaterFan off");
					controlFan(false);
					...
					if (c > setting.getTemperature() + 1) {
						controlHeater(false);
						System.out.println("c heaterFan off");
						controlFan(true);
						System.out.println("c fan on");

					} else if (c <= setting.getTemperature() + 1 && c >= setting.getTemperature() - 1) {
						controlFan(false);
						System.out.println("c fan off");
						controlHeater(false);
						System.out.println("c heaterFan off");

					} else if (c < setting.getTemperature() - 1) {
						controlFan(false);
						System.out.println("c fan off");
						controlHeater(true);
						System.out.println("c heaterFan on");
					}
				} else if (h < setting.getHumidity() - 3) {
					controlHumidified(true);
					controlFan(false);
					System.out.println("h fan off");
					controlHeater(false);
					System.out.println("h heaterFan off");
	...
	public void controlHumidified(boolean state) {
		String setOn = "sudo uhubctl -a 1 -l 1-1";
		String setOn2 = "sudo uhubctl -a 1 -l 2";
		String setOff = "sudo uhubctl -a 0 -l 1-1";
		String setOff2 = "sudo uhubctl -a 0 -l 2";
		...
		try {
			if (state == true) {
				process = Runtime.getRuntime().exec(setOn);
				process2 = Runtime.getRuntime().exec(setOn2);
				System.out.println("humidity start");
			} else {
				process = Runtime.getRuntime().exec(setOff);
				process2 = Runtime.getRuntime().exec(setOff2);
				System.out.println("humidity stop");
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public boolean notifyEmergency(Setting setting, Measurement measurement) {
		...
		try {
			...
			// Text
			message.setText("농작물 생장환경에 문제가 발생하였습니다.\n" + "현재 농작물 생장환경 --> \t" + "[온도]\t" + measurement.getTemperature()
					+ "℃\t" + " [습도]\t" + measurement.getHumidity() + "%\t" + " [co2]\t" + measurement.getCo2() + "ppm"
					+ "\n" + "농작물 생장환경 설정 --> \t" + "[온도]\t" + setting.getTemperature() + "℃\t" + " [습도]\t"
					+ setting.getHumidity() + "%\t" + " [co2]\t" + setting.getCo2() + "ppm"); // 메일 내용을 입력
			// send the message
